...
VOLUME_EMA_LEN = 14
# add gaussian noise to each candlestick
ADD_NOISE = True
# number of noise to add to each candlestick
NOISE_COUNT = 1
# add a unique for each stock to all its candlestick
ADD_ID = True
# ID for current stock data
ID = 0
# days of the week
day_list = ["Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sat"]
# numerical id for each day of the week
...
def downloadData(ticker):
    ...
    # get data
    try:
        df = yf.download(ticker, (dt.datetime.now() - dt.timedelta(days=200*1.4 + 50 + INPUT_CANDLE_COUNT*1.4)).strftime("%Y-%m-%d"), dt.datetime.now().strftime("%Y-%m-%d"))
    except:
        return None
...
def getLiveInput(stock_ticker, stock_id="built_in"):
    global INPUT_CANDLE_COUNT
    global ID

    if stock_id != "built_in":
        ID = stock_id

    # get data
    hist = getLiveData(stock_ticker)
